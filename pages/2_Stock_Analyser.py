...
{'Positive macro shock' if sc_sent>0 else 'Negative macro shock'} · Score: {sc_sent:+.1f}
    ...
    fig_sc.update_layout(**DC(),height=280,showlegend=False,
...
with sh2:
    ...
    if not all_q.empty and not nifty_df.empty:
        ...
        if beta_rows:
            ...
            fig_b.update_layout(**DC(),height=260,showlegend=False,
                title=dict(text=f"Predicted impact if Nifty {nifty_drop:+d}%",
                    font_size=12),margin=dict(l=4,r=4,t=32,b=4),
...
with fc2:
    ...
    fig_rbi.update_layout(**DCM(4,4,32,4),height=220,showlegend=True)
