# utils.py — shared data, models, and helpers for all pages
import warnings
...
def ohlcv(ticker, period="1y", interval="1d"):
    """OHLCV dataframe — 5 min cache."""
    try:
...
@st.cache_data(ttl=30)
def bulk_quotes(tickers_tuple):
    """Bulk quotes for screener — 30s cache. Pass tuple not list."""
    rows = []
...
def news():
    """News with sentiment scores — 10 min cache."""
    items = []
    for src, url in NEWS_FEEDS:
        try:
            ...
            for e in feed.entries[:7]:
                ...
                txt = e.title
                txt_l = txt.lower()
                bull  = sum(1 for w in BULLISH if w in txt_l)
                bear  = sum(1 for w in BEARISH if w in txt_l)
                total = bull + bear
                score = round((bull-bear)/max(total,1), 2)
                label = ("BULLISH" if score > 0.2 else
                         "BEARISH" if score < -0.2 else "NEUTRAL")
                event = next((k for k in SCENARIOS if k.lower()
                              in txt_l.replace("rate cut","RBI cuts 25bps")
                                      .replace("rate hike","RBI hikes 25bps")),
                             None)
                items.append({
                    "time":ts,"src":src,"txt":txt[:120],
                    "url":getattr(e,"link","#"),
                    "score":score,"label":label,"event":event,
                })
        except Exception:
            continue
...
def arima_forecast():
    """CPI ARIMA forecast — daily cache."""
    try:
...
@st.cache_data(ttl=86400)
def rbi_model():
    """Random Forest RBI rate decision — daily cache."""
    try:
        repo = np.array([
...
def indicators(df):
    """Compute RSI, MACD, Bollinger Bands on a Close series."""
    c = df["Close"]
    d = c.diff()
    ...
    return {
        ...
        "ma200": round(float(ma200.iloc[-1]),2) if not ma200.isna().all() else None,
...
def beta(stock_df, index_df):
    """Compute beta of stock vs index using last 252 days."""
    try:
        sr = stock_df["Close"].pct_change().dropna()
        ir = index_df["Close"].pct_change().dropna()
        aligned = pd.concat([sr,ir],axis=1,join="inner").dropna()
        aligned.columns = ["s","i"]
        cov = np.cov(aligned["s"], aligned["i"])
...
def momentum_score(df):
    """Composite momentum score for a stock DataFrame."""
    try:
        c = df["Close"]
        r1m = (c.iloc[-1]/c.iloc[-21]-1)*100  if len(c)>21  else 0
        r3m = (c.iloc[-1]/c.iloc[-63]-1)*100  if len(c)>63  else 0
        r6m = (c.iloc[-1]/c.iloc[-126]-1)*100 if len(c)>126 else 0
        vol = c.pct_change().tail(20).std()*100
        ind = indicators(df)
        rsi_v = ind["rsi"]
        score = (r1m*0.25 + r3m*0.20 + r6m*0.15
                 + max(0, (50-abs(rsi_v-50)))*0.25
                 - vol*0.15)
...
<style>
  ...
  .tile-price {{ font-size:19px; font-weight:700; color:{TEXT};
  ...
  .ntime {{ font-size:10px; color:{MUTED}; white-space:nowrap;
            min-width:34px; margin-top:2px; }}
  .nsrc  {{ font-size:9px; font-weight:700; color:{ACCENT};
            background:{ACCENT}22; padding:2px 6px; border-radius:3px;
            white-space:nowrap; margin-top:1px; }}
  .ntxt  {{ font-size:12px; color:{TEXT}; line-height:1.4; }}
  .nbull {{ font-size:9px; font-weight:700; color:{UP};
            background:{UP}22; padding:1px 5px; border-radius:3px;
            white-space:nowrap; margin-top:2px; }}
  .nbear {{ font-size:9px; font-weight:700; color:{DOWN};
            background:{DOWN}22; padding:1px 5px; border-radius:3px;
            white-space:nowrap; margin-top:2px; }}
  .nneut {{ font-size:9px; font-weight:700; color:{MUTED};
            background:{MUTED}22; padding:1px 5px; border-radius:3px;
            white-space:nowrap; margin-top:2px; }}
  ...
  .sc-sec  {{ font-size:12px; color:{TEXT}; min-width:80px; }}
  .sc-bar-fill {{ height:14px; border-radius:3px; }}
  .sc-val  {{ font-size:11px; font-weight:600; min-width:44px;
              text-align:right; }}
  ...
  /* Plotly chart bg */
  .js-plotly-plot .plotly .bg {{ fill:{BG2} !important; }}
</style>
